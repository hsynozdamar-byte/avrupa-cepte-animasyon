...
function selectScene(i){active=Math.max(0,Math.min(3,i));time=0;motionTime=0;document.querySelectorAll('.phone-wrap').forEach((el,j)=>{el.classList.toggle('active',j===active);el.inert=!all&&j!==active;});document.querySelectorAll('.scene-tab').forEach((el,j)=>{el.classList.toggle('selected',j===active);el.setAttribute('aria-current',j===active?'step':'false');});$('#note-title').innerHTML=scenes[active].noteTitle;$('#note-text').textContent=scenes[active].note;$('#caption').textContent=`0${active+1} / 04 · ${scenes[active].kind==='story'?'HİKAYE':'PREMIUM'}`;render(time);if(!matchMedia('(prefers-reduced-motion: reduce)').matches){phones[active].querySelector('.phone-art').animate([{opacity:0,transform:'translateY(10px) scale(.985)'},{opacity:1,transform:'translateY(0) scale(1)'}],{duration:600,easing:'cubic-bezier(.22,1,.36,1)'});if(active===3){const sheet=phones[3].querySelector('.modal-sheet-card');if(sheet)sheet.animate([{transform:'translateY(64px)',opacity:.4},{transform:'translateY(0)',opacity:1}],{duration:550,easing:'cubic-bezier(.22,1,.36,1)'});phones[3].querySelectorAll('.sheet-benefit-card').forEach((card,k)=>{card.animate([{opacity:0,transform:'translateY(14px) scale(.98)'},{opacity:1,transform:'translateY(0) scale(1)'}],{duration:450,delay:200+k*90,easing:'cubic-bezier(.22,1,.36,1)'});});}}}
